// 魔神ゆうとの笑ろてまうやろ！ 共通BGM管理モジュール(v13)
//
// ===== v12での修正(診断ログで実機から取得した実測値に基づく) =====
...
//
// ===== v13での追加修正(オープニングBGMとステージBGMが重なる問題) =====
// 「ゲームをアプリ切替画面からフォースクォート→再度開く→ステージを
// 開始する」と、以前のページのBGMと新しいページのBGMが同時に鳴る、
// という報告があった。フォースクォートであればSafariプロセス自体は
// 終了するはずだが、ページのJS側に後片付け(pagehideイベント)の機会が
// 一切与えられずに終了するケースがあり得るため、pagehideの発火だけに
// 頼らない片方向の安全策を追加した:ページが非表示になった瞬間
// (visibilitychange)に、理由を問わず即座にAudioContextをsuspend()して
// 無音化する(表示に戻った時にこちらから能動的に再開はしない。再開は
// 次のユーザー操作に任せる)。これにより、どのような終了経路であっても
// 「表示されていないページの音が鳴り続ける・後から復活する」ことを
// 構造的に防いでいる。
//
// ===== 診断ログ機能について =====
...
// 4. AudioContextの制御は片方向のみ能動的に行う:「非表示になったら
//    即座にsuspend()して無音化する」処理だけは入れているが、
//    「表示に戻ったら能動的にresume()する」処理はあえて入れていない
//    (再開は必ずユーザー操作をきっかけにしたresume()試行に任せる)。
//    「本当にページを離れる場合」(pagehideでpersistedでない場合)は
//    完全に閉じる。過去バージョンで双方向の能動的な制御
//    (非表示→suspend かつ 復帰→resume を両方自動で行う)を試した際に
//    実機で音量が下がったまま戻らない不具合が起きた経緯があるため、
//    「無音化する側」だけを能動的に行い、「鳴らす側」は必ず実際の
//    ユーザー操作を経由させることで、意図しない箇所で勝手に鳴り出す
//    ことも、能動的な制御同士が衝突することも避けている。
(function (global) {
  ...
  function initBgm(trackKey, basePath) {
    ...
    // ページが非表示になった瞬間、必ず即座に無音化する(片方向のみ:
    // 表示に戻った時にこちらから能動的に再開はしない。再開は次の
    // ユーザー操作(onUserGesture)に任せる)。
    // 【理由】force quit(アプリ切替画面からの完全終了)であっても、
    // ページのJS/AudioContextへ後片付け(pagehide)の機会が与えられずに
    // 終了する可能性がある。その場合でも「非表示になった瞬間に必ず
    // 無音化」しておけば、そのページの音が後から復活して別ページの
    // BGMと重なることを構造的に防げる。
